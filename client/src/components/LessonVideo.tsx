...
export function LessonVideo({ video }: { video: VideoContext | undefined }) {
  ...
  return <section className="lesson-video" aria-labelledby="video-title">
    <div className="video-heading"><div><span>VIDEO TÌNH HUỐNG · TUẦN {video.week}</span><h3 id="video-title">{video.title}</h3></div><div className="video-heading__actions">{video.externalSource && <a className="video-source-link" href={video.externalSource.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Mở video nguồn</a>}<button type="button" onClick={() => setShowCaptions((current) => !current)} aria-pressed={showCaptions}><Captions size={16} /> {showCaptions ? "Ẩn phụ đề" : "Hiện phụ đề"}</button></div></div>
    ...
    {video.externalSource && <p className="external-source-note"><b>Nguồn ngoài:</b> {video.externalSource.channel} · {video.externalSource.sourceTitle}. {video.externalSource.note}</p>}
